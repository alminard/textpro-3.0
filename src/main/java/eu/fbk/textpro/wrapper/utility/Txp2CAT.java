 ...
 * Time: 10.06.45
 *
 * ex: /research/hlt/cgirardi/bin/java/TextPro/txp2cat.sh -tokens -ein "ISO-8859-1" -eout "UTF8" -in /media/disk/hlt/Corpora/Adige/xmlFTP_clean/2008/ corpus/adige/2008/ corpus_xml/adige/2008/ > & /tmp/2008.log

 */
public class Txp2CAT {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Txp2CAT txp2xml = new Txp2CAT();
        if (args.length == 0) {
            System.err.println("java  eu.fbk.textpro.output.Txp2CAT [-tokens] [-ein encoding] [-eout encoding] [-in TextPro input file or dir] <txp file or dir> [XML output dir]\n\nFor default:\n-ein is "+encodingIN+"\n-eout is "+encodingOUT);
            ...
            if (txp_IN.isDirectory()) {
                ...
                for (int i=0; i<files.length; i++) {
                    if (files[i].isFile()) {
                        out = new File(xml_OUT,files[i].getName().replaceAll("\\.[^\\.]+","") +".xml");
    ...
    public int getXML (File txp_IN, File xml_OUT) throws Exception {
        ...
        doc.setAttribute("doc_name", txp_IN.getName());
        ...
        try {
            while ((line = buff.readLine()) != null) {
                ...
                if (line.startsWith("# ")) {
                    ...
                    if (saveTokens && cols.length > 0) {
                        token_counter++;
                        Element token = new Element("token");
                        token.setAttribute("id", String.valueOf(token_counter));
                        token.setAttribute("sentence", String.valueOf(sentence));
                        token.setAttribute("number", String.valueOf(token_counter-1));
                        token.setText(cols[0]);
                        doc.addContent(token);
                    }
                } else {
                    sentence++;
                }
