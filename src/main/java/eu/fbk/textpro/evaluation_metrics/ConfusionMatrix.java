package eu.fbk.textpro.evaluation_metrics;
...
public class ConfusionMatrix
{
	...
	Hashtable<String,Hashtable<String,Integer>> cm = new Hashtable<String, Hashtable<String,Integer>>();

    public ConfusionMatrix(String[] tags)
    {
    	for(String tg:tags){
    ...
    public Map<String, Double> getPrecisionForLabelsBIO()
    {
        Map<String, Double> precisions = new LinkedHashMap<>();
        for (Entry<String, Hashtable<String, Integer>> label : cm.entrySet()) {
            double precision = getPrecisionForLabelBIO(label.getKey());

            precisions.put(label.getKey(), precision);
        }
        return precisions;
    }
    ...
    public double getPrecisionForLabelBIO(String label)
    {
        double precision = 0;
        int tp = 0;
        int fpAndTp = 0;

        if (cm.containsKey(label)) {
            tp = this.cm.get(label).get("tp");
            fpAndTp = this.cm.get(label).get("tp")+this.cm.get(label).get("fp");;
        }
    ...
    /**
     * Returns Average F-measure for categories; see See http://en.wikipedia.org/wiki/F1_score
     *
     * @return double
     */
    public double getFMeasureAvg()
    {
        ...
        return (double)sum/labs;
    }
    public double getFMeasureAvgBIO()
    {
        Map<String, Double> fMeasure = new LinkedHashMap<>();

        Map<String, Double> precisionForLabels = getPrecisionForLabelsBIO();
        Map<String, Double> recallForLabels = getRecallForLabelsBIO();
        double sum=0;
		int labs=0;
        for (Entry<String, Hashtable<String, Integer>> label : cm.entrySet()) {
            double p = precisionForLabels.get(label.getKey());
            double r = recallForLabels.get(label.getKey());
    ...
    public Map<String, Double> getFMeasureForLabelsBIO()
    {
        Map<String, Double> fMeasure = new LinkedHashMap<>();
    ...
    public Map<String, Double> getRecallForLabelsBIO()
    {
        Map<String, Double> recalls = new LinkedHashMap<>();
        for (Entry<String, Hashtable<String, Integer>> label : cm.entrySet()) {
            double recall = getRecallForLabelBIO(label.getKey());

            recalls.put(label.getKey(), recall);
        }
        return recalls;
    }
    /**
    ...
    public double getRecallForLabelBIO(String label)
    {
        int fnAndTp = 0;
        double recall = 0;
        int tp = 0;
        ...
        if (cm.containsKey(label) ) {
            tp = this.cm.get(label).get("tp");
            fnAndTp = this.cm.get(label).get("tp")+this.cm.get(label).get("fn");
        }
    ...
    public String printLabelPrecRecFmBIO()
    {
        Map<String, Double> precisionForLabels = getPrecisionForLabelsBIO();
        Map<String, Double> recallForLabels = getRecallForLabelsBIO();
        Map<String, Double> fMForLabels = getFMeasureForLabelsBIO();
    ...
    public void parseBIO(String filepath) throws IOException{
	...
	while ((str = in.readLine()) != null) {
		str= str.trim();
		if(str.length()>0&&!str.startsWith("# FIELDS")){
			sentence.add(str);
		}else if(str.length()==0&&!str.startsWith("# FIELDS")){
			//calculate tp (gold+predict) fp(not gold but predict) fn (gold not predict) for the sentence
			sent++;
			LinkedList<NE> goldvsPredict = new LinkedList<>();
			LinkedList<NE> predictvsGold = new LinkedList<>();
			
			boolean startNEG=false,startNEP=false;
			for(int i=0;i<sentence.size();i++){
				String row=sentence.get(i);
				String[] cols = row.split("\t");
				if(!cols[1].startsWith("O")){
					if(startNEG&&cols[1].startsWith("I-")){
						//add new index to last entity
						goldvsPredict.getLast().indexs.add(sent+"-"+i);
					}else if(cols[1].startsWith("B-")){
						//new entity
						NE e = new NE();
						e.type=cols[1].replace("B-", "").trim();
						e.indexs.add(sent+"-"+i);
						goldvsPredict.add(e );
						startNEG=true;
					}else{
						//PROBLEM but i consider it as FP
						System.err.println("Problem GOLD entity without start B-: "+row);
						if(startNEG&&cols[1].startsWith("I-")){
							//new entity
							NE e = new NE();
							e.type=cols[1].replace("I-", "").trim();
							e.indexs.add(sent+"-"+i);
							goldvsPredict.add(e );
							startNEG=true;
						}
					}
				}else if(cols[1].equals("O")){
					startNEG=false;
				}
				
				
				if(!cols[2].startsWith("O")){
					if(startNEP&&cols[2].startsWith("I-")){
						//add new index to last entity
						predictvsGold.getLast().indexs.add(sent+"-"+i);
					}else if(cols[2].startsWith("B-")){
						//new entity
						NE e = new NE();
						e.type=cols[2].replace("B-", "").trim();
						e.indexs.add(sent+"-"+i);
						predictvsGold.add(e );
						startNEP=true;
					}else{
						//PROBLEM but i consider it as FP
						System.err.println("Problem SYS entity without start B-: "+row);
						if(startNEP&&cols[2].startsWith("I-")){
							//new entity
							NE e = new NE();
							e.type=cols[2].replace("I-", "").trim();
							e.indexs.add(sent+"-"+i);
							predictvsGold.add(e );
							startNEP=true;
						}
					}
				}else if(cols[2].equals("O")){
					startNEP=false;
				}
				
			}
			//System.out.println("Gold entities: "+goldvsPredict.size());
			//System.out.println("Predicted entities: "+predictvsGold.size());

			for(NE ne:goldvsPredict){
				 nothing=false;
				predictvsGold.stream()
				.filter(e->e.type.equals(ne.type)&&e.indexs.equals(ne.indexs))
				.forEach(e->{
						Hashtable<String, Integer> as = cm.get(ne.type);
						int count = as.get("tp").intValue();
						count+=1;
						as.put("tp", count);
						cm.put(ne.type, as);
						nothing=true;
						//System.out.println("####"+ne.type+" "+ne.indexs);
				});
				if(!nothing){
					Hashtable<String, Integer> as = cm.get(ne.type);
					int count = as.get("fn").intValue();
					count+=1;
					as.put("fn", count);
					cm.put(ne.type, as);
					//System.out.println("%%%%%%"+ne.type+" "+ne.indexs);
			...
			for(NE ne:predictvsGold){
				 nothing=false;
				 goldvsPredict.stream()
				.filter(e->e.type.equals(ne.type)&&e.indexs.equals(ne.indexs))
				.forEach(e->{
					nothing=true;
				});
				if(!nothing){
					Hashtable<String, Integer> as = cm.get(ne.type);
					int count = as.get("fp").intValue();
					count+=1;
					as.put("fp", count);
					cm.put(ne.type, as);
					//System.out.println("^^^^^"+ne.type+" "+ne.indexs+" count: "+as.get("fp")+" realCount: "+count);
				}
			}
	...
	//calculate P/R/F1 for each cat then for all avg
//	for( Entry<String, Hashtable<String, Integer>> ss:cm.entrySet()){
//		for(Entry<String, Integer> gg:ss.getValue().entrySet()){
//			System.out.println(ss.getKey()+" "+gg.getKey()+" "+gg.getValue());
//		}
//	}
	in.close();
    ...
    public static ConfusionMatrix parseGoldPredictColumns(String filepath)
            throws IllegalArgumentException
    {
        try {
        	List<String> lines = Files.readLines(new File(filepath), Charset.defaultCharset());
            String[] l = lines.get(0).replace("# FIELDS: ", "").trim().split("\t");
            ...
            for (int i = 1; i < lines.size(); i++) {
                ...
                String predictedLabel = split[2].trim();
                String acutalLabel = split[1].trim();
                result.increaseValue(predictedLabel, acutalLabel, 1);
                }
    ...
    public double getFMeasureOnPositiveNegative()
    {
        Map<String, Double> fMeasureForLabels = getFMeasureForLabels();
        double totalFMeasure = 0;
            totalFMeasure += fMeasureForLabels.getOrDefault("POSITIVO", 0.0);
            totalFMeasure += fMeasureForLabels.getOrDefault("NEGATIVO", 0.0);
        return totalFMeasure /2;
    }
    /**
